...
WaveletTree *wavelet_from_vec(const u32 *const restrict vals, const size_t n)
{
	...
	WaveletTree *const restrict wavelet = malloc(sizeof(*wavelet));
	if (wavelet == NULL)
		return NULL;
	...
	*wavelet = (WaveletTree){
	...
	u32 *const restrict temp = calloc(n, sizeof(*temp));
	if (temp == NULL) {
...
WaveletTree *wavelet_from_string(const char *const restrict str)
{
	...
	u32 *const restrict vals = calloc(n, sizeof(*vals));
...
u32 wavelet_at(const WaveletTree *const wavelet, const size_t i)
{
	...
	if (in_left)
		return wavelet_at(wavelet->left, rank);
